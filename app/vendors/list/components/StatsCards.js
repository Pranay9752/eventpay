  ...
  return (
    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
      {stats.map(({ title, value, icon, color }, index) => (
        ...
        >
          <div
            className={`absolute inset-0 bg-gradient-to-br from-${color}-50 to-${color}-100 dark:from-${color}-900/20 dark:to-${color}-800/10 z-0`}
          ></div>
          <CardHeader className="relative z-10 pb-2">
            <div className="flex justify-between items-start">
              <CardTitle
                className={`text-sm font-medium text-${color}-600 dark:text-${color}-300`}
              >
                {title}
              </CardTitle>
              {icon}
