            ...
            <div className="mt-3 text-center md:text-left">
              ...
              <div className="flex items-center gap-2 mt-1">
                ...
                <span className="text-sm text-gray-500">
                  ID: {vendor.vendorId} • Terminal: {vendor.terminalId}
                </span>
              </div>
            </div>
          </div>
          ...
          <div className="hidden md:flex flex-1 justify-end items-center mt-3">
            <div className="grid grid-cols-3 gap-3 w-full max-w-3xl">
              {/* <StatCard
                ...
                label="Transactions"
                value={vendor?.totalTransactions?.toString() || 0} 
                color="bg-blue-500"
              />
              <StatCard
                icon={<CreditCard className="w-5 h-5" />}
                label="Total Sales"
                value={`₹${vendor?.totalSales?.toLocaleString() || 0}`}
                color="bg-purple-500"
              /> */}
              {/* <StatCard 
...
function StatCard({ icon, label, value, color = "bg-blue-500" }) {
  return (
    <div className="p-4 rounded-xl bg-white border border-gray-100 shadow-sm hover:shadow-md transition-shadow duration-200 overflow-hidden relative group">
      <div
        className={`absolute -top-6 -left-6 w-12 h-12 rounded-full ${color} opacity-10 group-hover:scale-150 transition-transform duration-300`}
      ></div>
      <div
        className={`w-8 h-8 rounded-full ${color} bg-opacity-10 flex items-center justify-center mb-2`}
      >
